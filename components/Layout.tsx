...
const Layout: React.FC<LayoutProps> = ({
  ...
  const menuItems = [
    { id: "dashboard", label: "Dashboard", icon: <IconDashboard />, roles: [UserRole.ADMIN, UserRole.EXECUTIVE] },
    { id: "leads", label: "Leads", icon: <IconUsers />, roles: [UserRole.ADMIN, UserRole.EXECUTIVE] },
    { id: "tasks", label: "Tasks", icon: <IconClipboardList />, roles: [UserRole.ADMIN, UserRole.EXECUTIVE] },
    { id: "analytics", label: "Analytics", icon: <IconTrendingUp />, roles: [UserRole.ADMIN] }
  ];
      ...
      <aside className="w-64 glass border-r border-white/5 flex flex-col">
        ...
        <nav className="flex-1 px-4 py-4 space-y-2">
          {filteredMenu.map((item) => (
            ...
            >
              <div className={activeTab === item.id ? "text-[#06D001]" : ""}>{item.icon}</div>
